...
knapsack_dynamic(x = knapsack_objects[1:20,], W = 3500,fast = FALSE)
...
By successfully implementing Rcpp and c++ code in one chunk of the knapsack_dynamic function, the user time has reduced significantly. C++ code was used to execute a nested loop to create a matrix which is more faster than R loops. The above example shows by implementation and comparison before and anfter using the Rcpp and c++.
